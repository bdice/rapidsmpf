...
HostBuffer::HostBuffer(
    ...
    if (size > 0) {
        auto* ptr = static_cast<std::byte*>(mr_.allocate(stream_, size, alignof(::cuda::std::max_align_t)));
...
void HostBuffer::deallocate_async() noexcept {
    if (!span_.empty()) {
        ...
        if (owned_storage_) {
            ...
            mr_.deallocate(stream_, span_.data(), span_.size(), alignof(::cuda::std::max_align_t));
        }
    }
    span_ = {};
}
